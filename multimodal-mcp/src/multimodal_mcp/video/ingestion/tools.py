...
def extract_video_clip(
    video_path: str, start_time: float, end_time: float, output_path: str | Path
) -> VideoFileClip:
    """
    Extract a video clip between start_time and end_time using FFmpeg, then open it with MoviePy.
    """
    ...
    if result.returncode != 0:
        ...
        raise IOError(f"FFmpeg failed to extract clip: {result.stderr.strip()}")
...
def re_encode_video(video_path: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"An unexpected error occurred while trying to open video {video_path}: {e}"
        )
    finally:
        ...
        try:
            ...
            logger.info(
                f"FFmpeg re-encoding successful for {video_path} to {reencoded_video_path}"
            )
            ...
            try:
                with av.open(reencoded_video_path) as _:
                    logger.info(
                        f"Re-encoded video {reencoded_video_path} successfully opened by PyAV."
                    )
